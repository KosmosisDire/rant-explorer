/* The public interface of the live observer. net_capture.c is its own translation unit
   owning Rant and winsock, and the UI includes only this header of plain types. */
...
/* opaque to the UI: the fields are Rant runtime pointers, owned by net_capture.c */
typedef struct {
    void *rt;
    void *mem;
} Capture;
...
#define CAP_NAME_CAP   33    /* RANT_NODE_NAME_MAX (32) + NUL    */
#define CAP_TOPIC_CAP  65    /* RANT_TOPIC_NAME_MAX (64) + NUL */
...
/* Entity kind, values mirroring RantEntityKind. The capture walks peers through the entity
...
/* The mini value preview for the topic list's VALUE column (spec/explorer.md). std values
   mirror RantStdType, CAP_STD_NONE = not a standard type. */
enum {
    CAP_STD_NONE = 0,
...
typedef struct {
    ...
    int      call_status;            /* FUNCTION reply: its RantCallStatus, nonzero = the call
                                        failed. 0 on every other entry */
    char     call_msg[256];          /* FUNCTION reply: the response message, the provider's text or
                                        the default status text. Sized to RANT_CALL_MSG_MAX + 1 */
...
/* A field's type kind, values mirroring RantSchemaTypeKind so the UI can pick a type aware
...
enum {
    ...
    CAP_K_ENUM,   /* a named integer mirroring RANT_ENUM, a dropdown of variants */
...
typedef struct {
    ...
    int      call_status;        /* terminal RantCallStatus (phase DONE) */
...
/* Node logs: the explorer subscribes to all three @rant/log topics at start, so each
...
typedef struct {
    ...
    uint8_t  level;                   /* 0 error, 1 warn, 2 info (RantLogLevel) */
...
typedef struct {
    ...
    uint32_t last_error;              /* RantErrorKind (0 = none) */
    char     last_error_text[128];
    /* proc section: absent where the node's platform cannot measure (RANT_PROC_STATS off) */
    int      have_proc;
    int      have_cpu;
