...
/* the @rant/meta counter row for a topic name, or NULL if this snapshot lacks it */
...
static void nodes_runtime_sections(const Palette *P,
    ...
    ui_section_label(P, CLAY_STRING("RUNTIME  (@rant/meta)"));
    if (!fresh){
        CLAY({ .layout = { .sizing = { .width = CLAY_SIZING_GROW(0) }, .layoutDirection = CLAY_TOP_TO_BOTTOM },
            ...
            node_kv_single(P, CLAY_STRING("Status"),
                           (watching && ms->failing >= 2) ? CLAY_STRING("no @rant/meta response")
    ...
    CLAY({ .id = CLAY_ID("nodes_detail"),
      ...
      if (!D || D->n_nodes == 0 || app->sel_node < 0 || app->sel_node >= D->n_nodes){
        ...
        CLAY({ .id = CLAY_ID("nodes_detail_scroll"),
            ...
            /* RUNTIME: the node's own internals served by its @rant/meta endpoint, a directed
...
/* the right sidebar: the selected node's @rant/log stream, newest first, filtered by the
...
static void nodes_tab(AppState *app, const Palette *P){
    const Dataset *D = app->data;
    /* keep the 1 Hz @rant/meta poll aimed at the selected node */
    if (app->cap)
